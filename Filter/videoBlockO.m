...
%  for j=1:Nc*B
%   for k=1:fras
%    J1(i,j,k) = J(i,j,k)-I(i,j,k);
%    PSNRd = PSNRd + J1(i,j,k)^2;
%	PSNR = PSNR + I(i,j,k)^2;
%   end
%  end
%end
%PSNR = PSNRd/PSNR+1;
%%PSNR = PSNR/rows/cols/fras;
%PSNR = 10*log10(PSNR)
%%figure 3
%w = VideoWriter ("o-i.avi");
% open (w);
%   for k=1:fras
%     for j=1:Nc*B
%      for i=1:Nr*B
%       z(i,j,1)=z(i,j,2)=z(i,j,3)=J1(i,j,k);
%      end
%     end
%    zi = image (z);
%    drawnow
